...
export default function SlotMachine() {
  ...
  const toggleCouponSelection = (coupon) => {
    ...
    if (updatedSelections.length > 3) return;
  ...
  const payload = {
    value: {
      ...
      lightBulbColor,
      machineLogo,
      gameType: storeKeyEnum.slotMachine,
      emptyPrizeDisplayText
    },
    selectedCoupons,
    restaurantId: 1,
  ...
  const remainingCoupons = 3 - selectedCoupons.length;
  ...
  return (
    <Container>
      ...
      <h1>Select ({remainingCoupons}) Coupons</h1>
      <CouponsContainer>
        {coupons?.length > 0 &&
          coupons.map((coupon) => {
      ...
      <CreateGameButton onClick={() => createGame(payload, storeKeyEnum.slotMachine)}>Create Game</CreateGameButton>
    </Container>
  );
}
